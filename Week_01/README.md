...
4. MacOS 的 jmap 命令在jdk11上没有 jmap -heap [pid] 的选项了，换成Windows的jdk8进行测试。
5. ![image-20210115143922182](src/homework4/jmap.png)
6. jstack 查看一个 idea 运行一个死锁的 java 进程`jstack -l 7056 >> jstack.log`
...
1. Java5~8 使用的是使用的 Parallel GC，Java9~17 使用的G1GC。
2. 脱离业务场景谈性能都是刷流氓，具体的使用哪种GC需要结合实际业务进行选择和测试。
3. 串行 -> 并行: 重复利用多核 CPU 的优势，大幅降低 GC 暂停时间，提升吞吐量。
4.  并行 -> 并发： 不只开多个 GC 线程并行回收，还将GC操作拆分为多个步骤，让很多繁重的任务和应用线程一起并发执行，减少了单次 GC 暂停持续的时间，这能有效降低业务系统的延迟。 
5. CMS -> G1： G1 可以说是在 CMS 基础上进行迭代和优化开发出来的，划分为多个小堆块进行增量回收，这样就更进一步地降低了单次 GC 暂停的时间 
6. G1 -> ZGC:：ZGC 号称无停顿垃圾收集器，这又是一次极大的改进。ZGC 和 G1 有一些相似的地方，但是底层 的算法和思想又有了全新的突破。 
